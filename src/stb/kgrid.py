...
def print_density_recommendation():
    ...
    print()
    print_warn("For most systems, a density between 0.2 and 0.3 is")
...
def main():
    ...
    try:
        ...
        if file_type == 'cif':
            print_info(f"Reading file '{filename}' as type '{file_type}' (using ASE)...")
            lattice = parse_cif(filename)
            
        elif file_type == 'poscar':
            print_info(f"Reading file '{filename}' as type '{file_type}' (native method)...")
    ...
    print_ok(f"Suggested Monkhorst-Pack grid: {divisions[0]} {divisions[1]} {divisions[2]}")
    print()
